...
export const encrypt = (text: string) => {
  return CryptoJS.AES.encrypt(text, process.env.NEXT_PUBLIC_TEXT_ENCRYPT_KEY!).toString();
};

export const descrypt = (text: string) => {
  try {
    const bytes = CryptoJS.AES.decrypt(text, process.env.NEXT_PUBLIC_TEXT_ENCRYPT_KEY!);
