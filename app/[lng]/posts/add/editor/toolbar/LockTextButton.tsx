...
import AlertModal from "@/app/[lng]/components/modal/AlertModal";
...
const LockTextButton = ({ editor, isBlockSelected, blocks, setBlocks, activeNode }: Props) => {
	...
	const onLock = () => {
		...
		if (isActive) {
			if (activeNode.marks) {
				const mark = activeNode.marks[0];
				const securedId = mark.attrs.securedId;
